...
function handleGame() {
    if (game.getStatus() === (0, _gameClassDefault.default).STATUS.idle) {
...
function onMove(e) {
    ...
    renderBoard();
    if ([
        (0, _gameClassDefault.default).STATUS.win,
        (0, _gameClassDefault.default).STATUS.lose
    ].includes(game.getStatus())) {
        stopGame();
        manageMessage();
    }
}
...
function manageMessage() {
    switch(game.getStatus()){
        case (0, _gameClassDefault.default).STATUS.idle:
            startMessage.classList.remove("hidden");
            winMessage.classList.add("hidden");
            loseMessage.classList.add("hidden");
            break;
        case (0, _gameClassDefault.default).STATUS.playing:
            startMessage.classList.add("hidden");
            break;
        case (0, _gameClassDefault.default).STATUS.win:
            winMessage.classList.remove("hidden");
            break;
        case (0, _gameClassDefault.default).STATUS.lose:
 ...
 */ class Game {
   ...
   */ restart() {
    ...
    move(direction) {
        ...
        let newState;
        if (direction === Game.DIRECTION.left) newState = this.state.map((row)=>this.updateRow(row));
        if (direction === Game.DIRECTION.right) newState = this.reverseRows(this.reverseRows(this.state).map((row)=>{
            return this.updateRow(row);
        }));
        if (direction === Game.DIRECTION.up) newState = this.columnsToRows(this.columnsToRows(this.state).map((row)=>{
            return this.updateRow(row);
        }));
        if (direction === Game.DIRECTION.down) newState = this.columnsToRows(this.reverseRows(this.reverseRows(this.columnsToRows(this.state)).map((row)=>{
            return this.updateRow(row);
        })));
